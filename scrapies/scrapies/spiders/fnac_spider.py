...
class FnacSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        # Yield list pages.
        x_pagination = response.xpath('//ul[' + utils.xpath_class('Pager bottom-toolbar') + ']')
        if x_pagination:
            url_next_page = x_pagination.xpath('.//a[' + utils.xpath_class('prevnext actionNext') + ']/@href').extract_first()
            if url_next_page is not None:
                yield Request(url_next_page, callback=self.parse)

        # Yield product pages.
        x_list = response.xpath('//ul[' + utils.xpath_class('articleList') + ']')
        if x_list:
            urls = x_list.xpath('.//p[' + utils.xpath_class('Article-desc') + ']/a/@href').extract()
            for url in urls:
        ...
        # Yield product.
        x_product = response.xpath('//div[' + utils.xpath_class('f-productPage') + ']')
        if x_product:
            item = Product()


            # Categories
            x_categories = response.xpath('//ul[' + utils.xpath_class('f-breadcrumb') + ']')
            ...
            # Price
            x_price = response.xpath('//div[' + utils.xpath_class('f-priceBox') + ']')
            x_price_old = x_price.xpath('.//span[' + utils.xpath_class('f-priceBox-price f-priceBox-price--old') + ']')
            x_price_new = x_price.xpath('.//span[' + utils.xpath_class('f-priceBox-price f-priceBox-price--reco') + ']')
            ...
            # Avis
            x_avis = response.xpath('//div[' + utils.xpath_class('f-review-header') + ']')

            rate = x_avis.xpath('.//div[' + utils.xpath_class('f-review-headerRate') + ']/text()').extract_first()
            if rate is not None:
                rate = utils.string_to_float(rate.strip())

            max_rate = x_avis.xpath('.//span[' + utils.xpath_class('f-review-headerRateTotal') + ']/text()').extract_first()
